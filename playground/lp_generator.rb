...
items = CSV.open('items.csv', col_sep: ';', headers: :first_row).map(&:to_h)
pages = {}
items.each do |item|
...
pages.each do |page|
  content = template % page.last.slice(:title, :block)
  File.write("#{PAGES_DIRECTORY}/#{page.last[:filename]}", content)
end
